...
const resolvers : Resolvers = {
    Query: {
        seePhotoComments: async(_, { id, lastId } : SeePhotoCommentsArgs, { client } : Context) : Promise<SeePhotoCommentsResults[]> => {
            const comments = await client.photo.findUnique({ where: { id }, select: { comments: { select: { payload: true, user: true, id: true}, orderBy: { createdAt: "asc" } , take: 5, skip: lastId ? 1 : 0, ...(lastId && { cursor: { id: lastId }})}}})
